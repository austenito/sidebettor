class BetsController < ApplicationController
  ...
  def create        
    ...
    user_conditions[:user_id] = current_user.object_id
    
    # Set the user's bet ratio
    user_ratio = BetRatio.new(:ratio => user_conditions[:ratio].to_i)
    user_ratio.save
    user_conditions[:bet_ratio_id] = user_ratio.object_id
    ...
    challenger_conditions[:bet_ratio_id] = challenger_ratio.object_id
    ...
    prize_category.save
    prize = Prize.new(:prize_category_id => prize_category.object_id, :prize => params[:bet][:prize])
    prize.save
    
    # Create the bet
    title = params[:bet][:title]
    end_date = Date.today
    bet = Bet.new(:title => title, :end_date => end_date, :user_id => current_user.object_id, :prize_id => prize.object_id)
    bet.bet_conditions.push(user_condition)
    bet.bet_conditions.push(challenger_condition)
    
    # Create the bet requests
    user_request = BetRequest.new(:bet_id => bet.object_id, :user_id => current_user.object_id, :is_pending => false, :has_accepted => false)
    challenger_request = BetRequest.new(:bet_id => bet.object_id, :user_id => challenger_conditions[:user_id], :is_pending => true, :has_accepted => false)
    ...
    bet_status = BetStatus.new(:is_completed => false, :bet_id => bet.object_id)    
    bet_status.save
